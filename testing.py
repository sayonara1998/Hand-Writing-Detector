...
def softmax_forward(b):
    ...
    for i in range(len(b)):
        total += np.exp(b[i])
    final = [np.exp(b[i]) / total for i in range(len(b))]
    return final
...
def cross_entropy_forward(y,y_predicted):
    return -np.dot((np.log(y_predicted)).T,y)
...
def sigmoid_backward(z,g_z):
    ...
    for i in range(len(z_star)):
        new.append(z_star[i]-(z_star[i] * z_star[i]))
    newz = np.array(new)
    new = []
    for i in range(len(newz)):
        new.append(g_z[i] * newz[i])
    g_a = np.array(new)
    return g_a

def softmax_backward(y,y_predicted,g_zz):
    g_b = np.subtract(y_predicted,y)
    return g_b

def cross_entropy_backwards(y,y_predicted,J):
    g_z = np.subtract(y,y_predicted)
    return g_z

def NNBackward(x,y,alphaM,betaM,args):
    x, a, z, b, y_predicted, J = args
    g_zz = cross_entropy_backwards(y, y_predicted, J)
    g_b = softmax_backward(y, y_predicted,g_zz)
